...
function addRow() {
  const next: ShowEvent = {
    ...
    durationMs: 1000,
    func: "fuse:noop", // TODO: replace with real function enums
  };
  setEvents((e) => [...e, next]);
}
function removeRow(idx: number) {
  ...
  try {
    ...
    if (Array.isArray(json.events)) {
      ...
      setEvents(json.events.map((e: any) => ({ ...e, id: String(e.id) })));
    } else {
      setEvents([]);
    }
...
async function saveTimetable() {
  try {
    ...
    const payload = {
      ...
      duration_ms: tt?.duration_ms ?? events.reduce((acc, e) => acc + (e.durationMs ?? 0), 0),
      events,
      songs: songList,
    };
  ...
  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
    <h3 style={{ margin: 0 }}>Timetable & Events</h3>        {/* changed label text */}
    <button onClick={openNewEventEditor} style={{ padding: "6px 10px" }}>
      + Add Event
    </button>
  </div>
    ...
    <button onClick={loadTimetable}>Load JSON…</button>
        ...
        {tt ? (
          <div style={{ marginTop: 12, fontSize: 13 }}>
            <div><b>Target:</b> {tt.target}</div>
            <div><b>Duration:</b> {tt.duration_ms} ms</div>
            <div><b>Events:</b> {tt.events?.length ?? 0}</div>
          </div>
        ) : <p style={{ opacity: 0.7 }}>No timetable loaded.</p>}
        <hr style={{ margin: "12px 0" }} />
        <button onClick={buildBlob} disabled={!tt}>Build</button>
  ...
  <h4 style={{ marginTop: 16, marginBottom: 8 }}>Events ({events.length})</h4>
  <table style={{ width: "100%", fontSize: 12, borderCollapse: "collapse" }}>
    <thead>
      <tr>
        <th style={{ textAlign: "left", borderBottom: "1px solid #3a3d42", paddingBottom: 4 }}>#</th>
        <th style={{ textAlign: "left", borderBottom: "1px solid #3a3d42", paddingBottom: 4 }}>Song</th>
        <th style={{ textAlign: "left", borderBottom: "1px solid #3a3d42", paddingBottom: 4, width: 80 }}>Duration</th>
        <th style={{ textAlign: "left", borderBottom: "1px solid #3a3d42", paddingBottom: 4 }} />
      </tr>
    </thead>
    <tbody>
      {events.map((ev, idx) => (
        <tr key={ev.id}>
          ...
          <td style={{ paddingTop: 4, paddingBottom: 4 }}>{songList.find((s) => s.id === ev.songId)?.description ?? `Song ${ev.songId}`}</td>
          <td style={{ paddingTop: 4, paddingBottom: 4 }}>{ev.durationMs}ms</td>
          <td style={{ paddingTop: 4, paddingBottom: 4, textAlign: "right", display: "flex", gap: 4 }}>
            <button onClick={() => setEditingEvent({...ev})} style={{ padding: "2px 6px", fontSize: 11 }}>Edit</button>
            <button onClick={() => removeRow(idx)} style={{ padding: "2px 6px", fontSize: 11 }}>Delete</button>
          </td>
        </tr>
      ))}
    </tbody>
...
/>
      ...
      <div style={{ padding: 12 }}>
        ...
        <div style={{ height: 260, border: "1px dashed #3a3d42", borderRadius: 8, marginBottom: 12, display:"grid", placeItems:"center" }}>
  ...
  <ShowEventsEditor
    ...
    onSave={(evt: ShowEvent) => {
      setEvents((rows) => {
        ...
        if (idx >= 0) return rows.map((r) => (r.id === evt.id ? evt : r));
        return [...rows, evt];
      });
...
*/}
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={programTarget} disabled={!blob}>Program Device</button>
          <span style={{ fontSize: 12, opacity: 0.8 }}>{blob ? `${blob.length} bytes ready` : "Nothing built yet"}</span>
        </div>
      </div>
    </div>
  );
